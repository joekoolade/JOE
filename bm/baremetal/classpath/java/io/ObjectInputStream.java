/*
 * ObjectInputStream.java -- Class used to read serialized objects Copyright (C)
...
public class ObjectInputStream extends InputStream implements ObjectInput, ObjectStreamConstants {
  ...
  /**
   * Creates a new <code>ObjectInputStream</code> that will do all of its
   * reading from <code>in</code>. This method also checks the stream by
   * reading the header information (stream magic number and stream version).
   * 
   * @exception IOException
   *              Reading stream header from underlying stream cannot be
   *              completed.
   * 
   * @exception StreamCorruptedException
   *              An invalid stream magic number or stream version was read from
   *              the stream.
   * 
   ...
   * 
   * If an exception is thrown from this method, the stream is left in an
   * undefined state.
   * 
   * @exception ClassNotFoundException
   *              The class that an object being read in belongs to cannot be
   *              found.
   * 
   * @exception IOException
   *              Exception from underlying <code>InputStream</code>.
   */
  ...
  /**
   * This method makes a partial check of types for the fields contained given
   * in arguments. It checks primitive types of fields1 against non primitive
   * types of fields2. This method assumes the two lists has already been sorted
   * according to the Java specification.
   * 
   * @param name
   *          Name of the class owning the given fields.
   * @param fields1
   *          First list to check.
   * @param fields2
   *          Second list to check.
   * @throws InvalidClassException
   *           if a field in fields1, which has a primitive type, is a present
   *           in the non primitive part in fields2.
   */
  private void checkTypeConsistency(String name, ObjectStreamField[] fields1,
      ObjectStreamField[] fields2) throws InvalidClassException {
    int nonPrimitive=0;

    for (nonPrimitive=0; nonPrimitive < fields1.length && fields1[nonPrimitive].isPrimitive(); nonPrimitive++) {
    }
    ...
    while (i < fields2.length && nonPrimitive < fields1.length) {
      ...
      if (compVal < 0) {
        ...
        throw new InvalidClassException("invalid field type for " + f2.getName() + " in class "
            + name);
      }
    }
  }
  ...
  /**
   * This method reads a class descriptor from the real input stream and use
   * these data to create a new instance of ObjectStreamClass. Fields are sorted
   * and ordered for the real read which occurs for each instance of the
   * described class. Be aware that if you call that method you must ensure that
   * the stream is synchronized, in the other case it may be completely
   * desynchronized.
   * 
   * @return A new instance of ObjectStreamClass containing the freshly created
   *         descriptor.
   * @throws ClassNotFoundException
   *           if the required class to build the descriptor has not been found
   *           in the system.
   * @throws IOException
   *           An input/output error occured.
   * @throws InvalidClassException
   *           If there was a compatibility problem between the class present in
   *           the system and the serialized class.
   */
  protected ObjectStreamClass readClassDescriptor() throws ClassNotFoundException, IOException {
    ...
    for (int i=0; i < field_count; i++) {
      ...
      fields[i]=new ObjectStreamField(field_name, class_name, currentLoader);
    }

    /*
     * Now that fields have been read we may resolve the class (and read
    ...
    ObjectStreamField[] fieldmapping=new ObjectStreamField[2 * Math.max(stream_fields.length,
        real_fields.length)];
    ...
    /*
     * Check that there is no type inconsistencies between the lists. A special
     * checking must be done for the two groups: primitive types and not
     * primitive types.
     */
    ...
    while (stream_idx < stream_fields.length || real_idx < real_fields.length) {
      ...
      if (stream_idx == stream_fields.length) {
        ...
        int comp_val=real_fields[real_idx].compareTo(stream_fields[stream_idx]);
        ...
        if (comp_val < 0) {
          ...
          if (stream_field.getType() != real_field.getType())
            throw new InvalidClassException("invalid field type for " + real_field.getName()
                + " in class " + name);
        }
      }

      /*
       * If some of stream_fields does not correspond to any of real_fields, or
       * the opposite, then fieldmapping will go short.
       */
      if (map_idx == fieldmapping.length) {
        ObjectStreamField[] newfieldmapping=new ObjectStreamField[fieldmapping.length + 2];
  ...
  /**
   * Reads the current objects non-transient, non-static fields from the current
   * class from the underlying output stream.
   * 
   * This method is intended to be called from within a object's
   * <code>private void readObject (ObjectInputStream)</code> method.
   * 
   * @exception ClassNotFoundException
   *              The class that an object being read in belongs to cannot be
   *              found.
   ...
   *              This method was called from a context other than from the
   *              current object's and current class's
   *              <code>private void readObject (ObjectInputStream)</code>
   *              method.
   * 
   * @exception IOException
   *              Exception from underlying <code>OutputStream</code>.
   */
  ...
  /**
   * Registers a <code>ObjectInputValidation</code> to be carried out on the
   * object graph currently being deserialized before it is returned to the
   * original caller of <code>readObject ()</code>. The order of validation
   * for multiple <code>ObjectInputValidation</code> s can be controled using
   * <code>priority</code>. Validators with higher priorities are called
   * first.
   * 
   * @see java.io.ObjectInputValidation
   * 
   * @exception InvalidObjectException
   *              <code>validator</code> is <code>null</code>
   * 
   * @exception NotActiveException
   *              an attempt was made to add a validator outside of the
   *              <code>readObject</code> method of the object currently being
   *              deserialized
   */
  ...
  /**
   * Called when a class is being deserialized. This is a hook to allow
   * subclasses to read in information written by the
   * <code>annotateClass (Class)</code> method of an
   * <code>ObjectOutputStream</code>.
   * 
   * This implementation looks up the active call stack for a
   * <code>ClassLoader</code>; if a <code>ClassLoader</code> is found, it
   * is used to load the class associated with <code>osc</code>, otherwise,
   * the default system <code>ClassLoader</code> is used.
   * 
   * @exception IOException
   *              Exception from underlying <code>OutputStream</code>.
   * 
  ...
  protected Class resolveClass(ObjectStreamClass osc) throws ClassNotFoundException, IOException {
    return Class.forName(osc.getName(), true, null);
  }

  /**
   * Lookup a class stored in the local hashtable. If it is not use the global
   * lookup function in ObjectStreamClass to build the ObjectStreamClass. This
   * method is requested according to the behaviour detected in the JDK by
   * Kaffe's team.
   * 
   * @param clazz
   *          Class to lookup in the hash table or for which we must build a
   *          descriptor.
   * @return A valid instance of ObjectStreamClass corresponding to the
   *         specified class.
   */
   ...
   * {@link java.io.ObjectStreamClass.getObjectStreamClasses(java.lang.Class)}
   * does but using lookupClass instead of ObjectStreamClass.lookup. This dup is
   * necessary localize the lookup table. Hopefully some future rewritings will
   * be able to prevent this.
   * 
   * @param clazz
   *          This is the class for which we want the hierarchy.
   * 
  ...
  /**
   * Allows subclasses to resolve objects that are read from the stream with
   * other objects to be returned in their place. This method is called the
   * first time each object is encountered.
   * 
   * This method must be enabled before it will be called in the serialization
   * process.
   * 
   * @exception IOException
   *              Exception from underlying <code>OutputStream</code>.
  ...
  /**
   * If <code>enable</code> is <code>true</code> and this object is trusted,
   * then <code>resolveObject (Object)</code> will be called in subsequent
   * calls to <code>readObject (Object)</code>. Otherwise,
   * <code>resolveObject (Object)</code> will not be called.
   * 
  ...
  /**
   * Reads stream magic and stream version information from the underlying
   * stream.
   * 
   * @exception IOException
   *              Exception from underlying stream.
   * 
   * @exception StreamCorruptedException
   *              An invalid stream magic number or stream version was read from
   *              the stream.
  ...
  /**
   * This class allows a class to specify exactly which fields should be read,
   * and what values should be read for these fields.
   * 
  ...
  public static abstract class GetField {
    ...
    public abstract boolean get(String name, boolean defvalue) throws IOException,
        IllegalArgumentException;

    public abstract char get(String name, char defvalue) throws IOException,
        IllegalArgumentException;

    public abstract byte get(String name, byte defvalue) throws IOException,
        IllegalArgumentException;

    public abstract short get(String name, short defvalue) throws IOException,
        IllegalArgumentException;

    public abstract int get(String name, int defvalue) throws IOException, IllegalArgumentException;

    public abstract long get(String name, long defvalue) throws IOException,
        IllegalArgumentException;

    public abstract float get(String name, float defvalue) throws IOException,
        IllegalArgumentException;

    public abstract double get(String name, double defvalue) throws IOException,
        IllegalArgumentException;

    public abstract Object get(String name, Object defvalue) throws IOException,
        IllegalArgumentException;
  }
   ...
   * outside of it. Its goal is to read all fields in the real input stream and
   * keep them accessible through the {@link #GetField}class. Calling this
   * method will not alter the deserializing object.
   * 
   * @return A valid freshly created 'GetField' instance to get access to the
   *         deserialized stream.
   * @throws IOException
   ...
   * This constructor should be called by subclasses that wish to override
   * <code>readObject (Object)</code>. This method does a security check
   * <i>NOTE: currently not implemented </i>, then sets a flag that informs
   ...
   * mechanism provided by <code>ObjectInputStream</code>. To make this
   * method be used for writing objects, subclasses must invoke the 0-argument
   ...
   * 
   * @param obj
   *          The object for which we want a new handle.
  ...
  private void readArrayElements(Object array, Class clazz) throws ClassNotFoundException,
      IOException {
  ...
  private void readFields(Object obj, ObjectStreamClass stream_osc) throws ClassNotFoundException,
      IOException {
    ...
    for (int i=0; i < fields.length; i+=2) {
      ...
      boolean read_value=(stream_field != null && stream_field.getOffset() >= 0 && stream_field
          .isToSet());
      ...
      switch (type) {
      case 'Z' : {
        boolean value=read_value ? this.realInputStream.readBoolean() : false;
      ...
      case 'B' : {
        byte value=read_value ? this.realInputStream.readByte() : 0;
        if (dump && read_value && set_value)
          dumpElementln("  " + field_name + ": " + value);
        if (set_value)
      ...
      case 'C' : {
        char value=read_value ? this.realInputStream.readChar() : 0;
        if (dump && read_value && set_value)
          dumpElementln("  " + field_name + ": " + value);
        if (set_value)
      ...
      case 'D' : {
        double value=read_value ? this.realInputStream.readDouble() : 0;
        if (dump && read_value && set_value)
          dumpElementln("  " + field_name + ": " + value);
        if (set_value)
      ...
      case 'F' : {
        float value=read_value ? this.realInputStream.readFloat() : 0;
        if (dump && read_value && set_value)
          dumpElementln("  " + field_name + ": " + value);
        if (set_value)
      ...
      case 'I' : {
        int value=read_value ? this.realInputStream.readInt() : 0;
        if (dump && read_value && set_value)
          dumpElementln("  " + field_name + ": " + value);
        if (set_value)
      ...
      case 'J' : {
        long value=read_value ? this.realInputStream.readLong() : 0;
        if (dump && read_value && set_value)
          dumpElementln("  " + field_name + ": " + value);
        if (set_value)
      ...
      case 'S' : {
        short value=read_value ? this.realInputStream.readShort() : 0;
        if (dump && read_value && set_value)
          dumpElementln("  " + field_name + ": " + value);
        if (set_value)
      ...
      case 'L' :
      case '[' : {
        Object value=read_value ? readObject() : null;
        if (set_value)
  ...
  private Object newObject(Class real_class, Class constructor_class) throws ClassNotFoundException {
    try {
      Object obj=allocateObject(real_class);
      callConstructor(constructor_class, obj);
      return obj;
    } catch (InstantiationException e) {
      throw new ClassNotFoundException("Instance of " + real_class + " could not be created");
    }
  }
  ...
  private void callReadMethod(Method readObject, Class klass, Object obj) throws IOException {
    try {
      ...
      throw new IOException("Exception thrown from readObject() on " + klass + ": "
          + exception.getClass().getName());
    } catch (Exception x) {
      throw new IOException("Failure invoking readObject() on " + klass + ": "
          + x.getClass().getName());
    }
  ...
  private Object allocateObject(Class clazz) throws InstantiationException {
