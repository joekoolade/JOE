/*
 * ObjectStreamClass.java -- Class used to write class information about
 * serialized objects. Copyright (C) 1998, 1999, 2000, 2001, 2003 Free Software
 * Foundation, Inc.
 * 
...
public class ObjectStreamClass implements Serializable {
  ...
  /**
   * Returns the <code>ObjectStreamClass</code> for <code>cl</code>. If
   * <code>cl</code> is null, or is not <code>Serializable</code>, null is
   * returned. <code>ObjectStreamClass</code>'s are memorized; later calls to
   * this method with the same class will return the same
   * <code>ObjectStreamClass</code> object and no recalculation will be done.
   * 
  ...
  /**
   * This lookup for internal use by ObjectOutputStream. Suppose we have a
   * java.lang.Class object C for class A, though A is not serializable, but
   * it's okay to serialize C.
   */
  ...
  /**
   * Returns the name of the class that this <code>ObjectStreamClass</code>
   * represents.
   */
  ...
  /**
   * Returns the class that this <code>ObjectStreamClass</code> represents.
   * Null could be returned if this <code>ObjectStreamClass</code> was read
   * from an <code>ObjectInputStream</code> and the class it represents cannot
   * be found or loaded.
   * 
   ...
   * represented by this <code>ObjectStreamClass</code>. This SUID is either
   * defined by the class as <code>static final long
   * serialVersionUID</code>
   * or is calculated as specified in Javasoft's "Object Serialization
   * Specification" XXX: add reference
   */
  ...
  /**
   * Returns a textual representation of this <code>ObjectStreamClass</code>
   * object including the name of the class it represents as well as that
   * class's serial version stream-unique identifier.
   * 
   ...
   * instance, that method sets the serial UID, chose the fields which will be
   * serialized, and compute the position of the fields in the serialized
   * stream.
   * 
   * @param cl
   *          The Java class which is used as a reference for building the
   *          descriptor.
   * @param superClass
   *          The descriptor of the super class for this class descriptor.
   * @throws InvalidClassException
   *           if an incompatibility between computed UID and already set UID is
   *           found.
   */
...
//    cacheMethods();
    ...
    if (uid == 0)
      ...
      // the stream.
      if (uid != class_uid) {
        String msg=cl + ": Local class not compatible: stream serialVersionUID=" + uid
    ...
    try {
      ...
      /*
       * We now check the import fields against the exported fields. There
       * should not be contradiction (e.g. int x and String x) but extra virtual
       * fields can be added to the class.
       */
      ...
      while (i < fields.length && j < exportedFields.length) {
        ...
        if (comp < 0) {
          ...
          /*
           * field not found in imported fields. We add it in the list of
           * supported fields.
           */
  ...
  private void setFlags(Class cl) {
    ...
    try {
      ...
      if (writeMethod.getReturnType() == Void.TYPE && Modifier.isPrivate(modifiers)
   ...
   * `serialPersistentFields'. It performs some sanity checks before returning
   * the real array. Besides, the returned array is a clean copy of the
   * original. So it can be modified.
   * 
   * @param clazz
   *          Class to retrieve 'serialPersistentFields' from.
   * @return The content of 'serialPersistentFields'.
   */
  private ObjectStreamField[] getSerialPersistentFields(Class clazz) throws NoSuchFieldException,
    ...
    if (!(Modifier.isStatic(modifiers) && Modifier.isFinal(modifiers) && Modifier
        .isPrivate(modifiers)))
      return null;
  ...
  /*
   * fixme: this will be a total rewrite!
   */
  private static Hashtable classLookupTable=new Hashtable();
  private static final NullOutputStream nullOutputStream=null; //new
                                                               // NullOutputStream();
  private static final Comparator interfaceComparator=null; //new
                                                            // InterfaceComparator();
  private static final Comparator memberComparator=null; //new
                                                         // MemberComparator();
  private static final Class[] writeMethodArgTypes=null; //{
