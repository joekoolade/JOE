...
public abstract class ResourceBundle {
  ...
  /**
   * The parent bundle. This is consulted when you call getObject and there is
   * no such resource in the current bundle. This field may be null.
   */
  ...
  /**
   * The resource bundle cache. This is a two-level hash map: The key is the
   * class loader, the value is a new HashMap. The key of this second hash map
   * is the localized name, the value is a soft references to the resource
   * bundle.
   */
  private static Map resourceBundleCache;

  /**
   * The last default Locale we saw. If this ever changes then we have to reset
   * our caches.
   */
  ...
  /**
   * Get a String from this resource bundle. Since most localized Objects are
   * Strings, this method provides a convenient way to get them without casting.
   * 
   * @param key
  ...
  /**
   * Get an array of Strings from this resource bundle. This method provides a
   * convenient way to get it without casting.
   * 
   * @param key
   *          the name of the resource
   * @throws MissingResourceException
   ...
   * getBundle, to know if the bundle for the desired locale was loaded or if
   * the fall back was used.
   * 
  ...
  public static final ResourceBundle getBundle(String baseName) {
    return getBundle(baseName, Locale.getDefault(), getCallingClassLoader());
  }
   ...
   * 
   * <p>
   * A sequence of candidate bundle names are generated, and tested in this
   * order, where the suffix 1 means the string from the specified locale, and
   * the suffix 2 means the string from the default locale:
   * <ul>
   ...
   * <code>getBundle</code> tries to instantiate the resource bundle:
   * <ul>
   ...
   * with '/' and appending ".properties", and using ClassLoader.getResource().
   * If a file is found, then a PropertyResourceBundle is created from the
   * file's contents.</li>
   * </ul>
   * If no resource bundle was found, a MissingResourceException is thrown.
   * 
   * <p>
   * Next, the parent chain is implemented. The remaining candidate names in the
   * above sequence are tested in a similar manner, and if any results in a
   * resource bundle, it is assigned as the parent of the first bundle using the
   * <code>setParent</code> method (unless the first bundle already has a
   * parent).
   * 
   * <p>
   * For example, suppose the following class and property files are provided:
   * MyResources.class, MyResources_fr_CH.properties, MyResources_fr_CH.class,
   * MyResources_fr.properties, MyResources_en.properties, and
   * MyResources_es_ES.class. The contents of all files are valid (that is,
   * public non-abstract subclasses of ResourceBundle with public nullary
   * constructors for the ".class" files, syntactically correct ".properties"
   * files). The default locale is Locale("en", "UK").
   * 
   * <p>
  ...
  // handled.
  public static final synchronized ResourceBundle getBundle(String baseName, Locale locale,
    ...
    // base bundle.
    ResourceBundle baseBundle=tryBundle(baseName, emptyLocale, classLoader, null, cache);
    ...
    // bundle.
    ResourceBundle bundle=tryLocalBundle(baseName, locale, classLoader, baseBundle, cache);
    if (bundle == baseBundle && !locale.equals(defaultLocale)) {
      bundle=tryLocalBundle(baseName, defaultLocale, classLoader, baseBundle, cache);
  ...
  /**
   * Override this method to provide the resource for a keys. This gets called
   * by <code>getObject</code>. If you don't have a resource for the given
   * key, you should return null instead throwing a MissingResourceException.
   * You don't have to ask the parent, getObject() already does this; nor should
   * you throw a MissingResourceException.
   * 
   * @param key
  ...
  /**
   * This method should return all keys for which a resource exists; you should
   * include the enumeration of any parent's keys, after filtering out
   * duplicates.
   * 
   ...
   *          the name
   * @param locale
   *          the locale, that must be used exactly
   * @param classloader
   *          the classloader
   * @param bundle
  ...
  private static final ResourceBundle tryBundle(String localizedName, Locale locale,
      ClassLoader classloader, ResourceBundle bundle, HashMap cache) {
    ...
    if (foundBundle == null) {
      try {
        InputStream is;
        final String resourceName=localizedName.replace('.', '/') + ".properties";
        if (classloader == null)
  ...
  private static final ResourceBundle tryLocalBundle(String baseName, Locale locale,
    ...
    if (language.length() > 0) {
      sb.append(language);
      bundle=tryBundle(sb.toString(), new Locale(language), classloader, bundle, cache);
    }
    ...
    if (country.length() > 0) {
      sb.append(country);
      bundle=tryBundle(sb.toString(), new Locale(language, country), classloader, bundle, cache);
    }
    sb.append('_');

    if (variant.length() > 0) {
      sb.append(variant);
      bundle=tryBundle(sb.toString(), locale, classloader, bundle, cache);
    }
