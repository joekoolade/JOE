...
public class Throwable implements Serializable {
   ...
   * Then for all elements given by <code>getStackTrace</code> it prints a
   * line containing three spaces, the string "at " and the result of calling
   ...
   * a line containing three spaces and the string "&lt;&lt;No stacktrace
   * available&gt;&gt;". <br>
   * Then if <code>getCause()</code> doesn't return null it adds a line
   * starting with "Caused by: " and the result of calling
   * <code>toString()</code> on the cause. <br>
   * Then for every cause (of a cause, etc) the stacktrace is printed the same
   * as for the top level <code>Throwable</code> except that as soon as all
   * the remaining stack frames of the cause are the same as the the last stack
  ...
  public Throwable fillInStackTrace() {
    vmState=null; // VMThrowable.fillInStackTrace(this);
