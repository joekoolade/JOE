 ...
 * It provides the operations required to modify the <code>StringBuffer</code>,
 * including insert, replace, delete, append, and reverse. It is thread-safe;
 * meaning that all modifications to a buffer are in synchronized methods.
 * 
 * <p>
 * <code>StringBuffer</code> s are variable-length in nature, so even if you
 ...
 * <code>StringBuffer</code> can have in it before it has to grow (growing the
 ...
 * <code>StringBuffer</code> operation: <br>
 * <code>a + b</code><br>
 * is the same as <br>
 ...
 * efficiency. This will help when a StringBuffer is converted to a String and
 * the StringBuffer is not changed after that (quite common when performing
...
public final class StringBuffer implements Serializable, CharSequence {
  ...
  /**
   * Index of next available character (and thus the size of the current string
   * contents). Note that this has permissions set this way so that String can
   * get the value.
   * 
  ...
  /**
   * The buffer. Note that this has permissions set this way so that String can
   * get the value.
   * 
   * @serial the buffer
   */
  ...
  /**
   * True if the buffer is shared with another object (StringBuffer or String);
   * this means the buffer must be copied before writing to it again. Note that
   * this has permissions set this way so that String can get the value.
   * 
  ...
  /**
   * Set the length of this StringBuffer. If the new length is greater than the
   * current length, all the new characters are set to '\0'. If the new length
   * is less than the current length, the first <code>newLength</code>
   * characters of the old array will be preserved, and the remaining characters
   * are truncated.
   * 
   * @param newLength
   *          the new length
  ...
  public synchronized void setLength(int newLength) {
    ...
    if (newLength < valueLength) {
      /*
       * If the StringBuffer's value just grew, then we know that value is newly
       * allocated and the region between count and newLength is filled with
       * '\0'.
       */
      count = newLength;
    } else {
      /*
       * The StringBuffer's value doesn't need to grow. However, we should clear
       * out any cruft that may exist.
       */
   ...
   * @throws IndexOutOfBoundsException
   *           if index is negative or &gt;= length() (while unspecified, this
   ...
   * <code>StringBuffer</code>. Uses <code>String.valueOf()</code> to
   * convert to <code>String</code>.
   * 
   * @param obj
   ...
   * <code>end</code> (exclusive) with <code>str</code>. If
   * <code>end</code> is larger than the size of this StringBuffer, all
   * characters after <code>start</code> are replaced.
   * 
   * @param start
  ...
  /**
   * Creates a substring of this StringBuffer, starting at a specified index and
   * ending at the end of this StringBuffer.
   * 
   * @param beginIndex
  ...
  /**
   * Creates a substring of this StringBuffer, starting at a specified index and
   * ending at one character before a specified index. This is implemented the
   * same as <code>substring(beginIndex, endIndex)</code>, to satisfy the
   ...
   * beginning of this String. If the starting index is greater than the length
   * of this String, or the substring is not found, -1 is returned.
   * 
   * @param str
   *          String to find
   * @param fromIndex
   ...
   * then the search begins at the end of this String. If the starting index is
   * less than zero, or the substring is not found, -1 is returned.
   * 
   * @param str
   *          String to find
   * @param fromIndex
   ...
   * that the result is a copy, and that future modifications to this buffer do
   * not affect the String.
   * 
   ...
   * cost of a second lock on the same object. This also has the side effect of
   * duplicating the array, if it was shared (to form copy-on-write semantics).
   * 
   * @param minimumCapacity
