...
/**
 * ThreadGroup allows you to group Threads together. There is a hierarchy of
 * ThreadGroups, and only the initial ThreadGroup has no parent. A Thread may
 * access information about its own ThreadGroup, but not its parents or others
 * outside the tree.
 * 
 * @author John Keiser
 * @author Tom Tromey
...
public class ThreadGroup {
  ...
  /**
   * This flag is set if an uncaught exception occurs. The runtime should check
   * this and exit with an error status if it is set.
   */
   ...
   * group inherits the maximum priority and daemon status of its parent group.
  ...
  /**
   * Get the parent of this ThreadGroup. If the parent is not null, there may be
   * a security check, <code>checkAccess</code>.
   * 
  ...
  /**
   * Tell whether this ThreadGroup is a daemon group. A daemon group will be
   * automatically destroyed when its last thread is stopped and its last thread
   * group is destroyed.
   * 
   ...
   * @throws SecurityException
   *           if you cannot modify this ThreadGroup
   * @see #checkAccess()
   */
   ...
   * can only be used to reduce the current maximum. If maxpri is greater than
   * the current Maximum of the parent group, the current value is not changed.
   * Otherwise, all groups which belong to this have their priority adjusted as
   * well. Calling this does not affect threads already in this ThreadGroup.
   * There may be a security check, <code>checkAccess</code>.
   * 
   * @param maxpri
  ...
  /**
   * Check whether this ThreadGroup is an ancestor of the specified ThreadGroup,
  ...
  /**
   * Find out if the current Thread can modify this ThreadGroup. This passes the
   * check on to <code>SecurityManager.checkAccess(this)</code>.
   * 
   * @throws SecurityException
   *           if the current Thread cannot modify this ThreadGroup
   * @see SecurityManager#checkAccess(ThreadGroup)
   */
  public final void checkAccess() {
    // Bypass System.getSecurityManager, for bootstrap efficiency.
    /*
     * SecurityManager sm = Runtime.securityManager; if (sm != null)
     * sm.checkAccess(this);
     */}
   ...
   * should be pretty close. Based on a JDC bug, <a
   * href="http://developer.java.sun.com/developer/bugParade/bugs/4089701.html">
   ...
   * into the specified array. If the array is not big enough to hold all the
   * Threads, extra Threads will simply not be copied. There may be a security
   ...
   *           if a thread does not fit in the array
   * @see #activeCount()
   * @see #checkAccess()
  ...
  /**
   * Copy all of the active Threads from this ThreadGroup and, if desired, from
   * its descendants, into the specified array. If the array is not big enough
   * to hold all the Threads, extra Threads will simply not be copied. There may
  ...
  /**
   * Get the number of active groups in this ThreadGroup. This group itself is
   * not included in the count. A sub-group is active if it has not been
   * destroyed. This cannot return an exact number, since the status of threads
   * may change after they were counted; but it should be pretty close.
   * 
  ...
  /**
   * Copy all active ThreadGroups that are descendants of this ThreadGroup into
   * the specified array. If the array is not large enough to hold all active
   * ThreadGroups, extra ThreadGroups simply will not be copied. There may be a
   ...
   *           if array is null
   * @throws ArrayStoreException
   *           if a group does not fit in the array
   * @see #activeCount()
   * @see #checkAccess()
  ...
  /**
   * Copy all active ThreadGroups that are children of this ThreadGroup into the
   * specified array, and if desired, also all descendents. If the array is not
   * large enough to hold all active ThreadGroups, extra ThreadGroups simply
   * will not be copied. There may be a security check, <code>checkAccess</code>.
   * 
   * @param array
   *          the array to put the ThreadGroups into
   * @param recurse
  ...
  /**
   * Interrupt all Threads in this ThreadGroup and its sub-groups. There may be
   * a security check, <code>checkAccess</code>.
   * 
   * @throws SecurityException
   *           if permission is denied
   ...
   * 
   * <p>
   * This is inherently unsafe, as suspended threads still hold locks, which can
   * lead to deadlock. Hence, there is a security check:
   * <code>checkAccess()</code>, followed by further checks on each thread
   * being suspended.
   * 
   * @throws SecurityException
  ...
  /**
   * Resume all suspended Threads in this ThreadGroup and its descendants. To
   * mirror suspend(), there is a security check: <code>checkAccess()</code>,
   * followed by further checks on each thread being resumed.
   * 
   * @throws SecurityException
   *           if permission is denied
   * @see #checkAccess()
  ...
  /**
   * Destroy this ThreadGroup. The group must be empty, meaning that all threads
   * and sub-groups have completed execution. Daemon groups are destroyed
   * automatically. There may be a security check, <code>checkAccess</code>.
   * 
   * @throws IllegalThreadStateException
   *           if the ThreadGroup is not empty, or was previously destroyed
   * @throws SecurityException
   *           if permission is denied
   * @see #checkAccess()
   */
  ...
  /**
   * Print out information about this ThreadGroup to System.out. This is meant
   * for debugging purposes. <b>WARNING: </b> This method is not secure, and can
   * print the name of threads to standard out even when you cannot otherwise
   * get at such threads.
   */
  public void list() {
    list("");
  ...
  /**
   * When a Thread in this ThreadGroup does not catch an exception, the virtual
   * machine calls this method. The default implementation simply passes the
   * call to the parent; then in top ThreadGroup, it will ignore ThreadDeath and
   * print the stack trace of any other throwable. Override this method if you
   * want to handle the exception in a different manner.
   * 
   * @param thread
  ...
  /**
   * Originally intended to tell the VM whether it may suspend Threads in low
   * memory situations, this method was never implemented by Sun, and is hence a
   * no-op.
   * 
   * @param allow
  ...
  /**
   * Return a human-readable String representing this ThreadGroup. The format of
   * the string is: <br>
   ...
   *          the array to put the threads into
   * @param next
   *          the next open slot in the array
   * @param recurse
   *          whether to recurse into descendent ThreadGroups
   * @return the number of threads put into the array
   * @throws SecurityException
   ...
   * 
   * @param list
   *          the array to put the groups into
   * @param next
   *          the next open slot in the array
   * @param recurse
   *          whether to recurse into descendent ThreadGroups
