...
public abstract class BytesToUnicode extends IOConverter {
  ...
  /**
   * Buffer to read bytes from. The characters inbuffer[inpos] ...
   * inbuffer[inlength-1] are available.
   */
  ...
  /*
   * These keep a small cache of decoders for reuse. The array holds the actual
   * decoders. The currCachePos is the next value we are going to replace in the
   * cache. We don't just throw the data away if the cache is full, because if
   * the cache filled up with stuff we don't need then the cache would be
   * worthless. We instead circulate through the cache the implement kind of an
   * LRU algorithm.
   */
      ...
      synchronized (BytesToUnicode.class) {
        if (defaultEncoding == null) {
          ...
          try {
            ...
            throw new NoClassDefFoundError("missing default encoding " + encoding + " (class "
                + className + " not found)");
          }
        }
      }
  ...
  public static BytesToUnicode getDecoder(String encoding)
    ...
    /*
     * First hunt in our cache to see if we have a decoder that is already
     * allocated.
     */
    synchronized (BytesToUnicode.class) {
      int i;
      for (i=0; i < decoderCache.length; ++i) {
        if (decoderCache[i] != null && encoding.equals(decoderCache[i].getName())) {
    ...
    try {
      ...
      try {
        ...
        throw new java.io.UnsupportedEncodingException(encoding + " (" + ex + ')');
      }
    }
  }
  ...
  /**
   * Convert bytes to chars. Input bytes are taken from this.inbuffer. The
   * available input bytes start at inbuffer[inpos], and end at
   * inbuffer[inlength-1].
   * 
   * @param outbuffer
   *          buffer for the converted character
   * @param outpos
   *          position in buffer to start putting converted characters
   * @param count
   *          the maximum number of characters to convert
   * @return number of chars placed in outbuffer. Also, this.inpos is
   *         incremented by the number of bytes consumed.
   * 
   ...
   * BufferedInputStream, while the count parameter is like the length parameter
   * of a read request.) The count parameter is also defined to be <=
   * outbuffer.length - outpos (per the specification of the length parameter
   * for a read request).
   */
  public abstract int read(char[] outbuffer, int outpos, int count);

  /**
   * Indicate that the converter is resuable. This class keeps track of
   * converters on a per-encoding basis. When done with an encoder you may call
   * this method to indicate that it can be reused later.
   */
  public void done() {
    synchronized (BytesToUnicode.class) {
      this.inbuffer=null;
