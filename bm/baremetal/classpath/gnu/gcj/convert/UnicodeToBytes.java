...
import baremetal.platform.Boot;

public abstract class UnicodeToBytes extends IOConverter
{

  /**
   * Buffer to emit bytes to. The locations buf[count] ... buf[buf.length-1]
   * are available.
   */
  public byte[] buf;
  public int count;
  ...
  /*
   * These keep a small cache of encoders for reuse. The array holds the actual
   * encoders. The currCachePos is the next value we are going to replace in
   * the cache. We don't just throw the data away if the cache is full, because
   * if the cache filled up with stuff we don't need then the cache would be
  ...
  /**
   * Convert chars to bytes. Converted bytes are written to buf, starting at
   * count.
   * 
   * @param inbuffer
   *          source of characters to convert
   * @param inpos
   *          index of initial character in inbuffer to convert
   * @param inlength
   *          number of characters to convert
   * @return number of chars converted Also, this.count is increment by the
   ...
   *          source of characters to convert
   * @param inpos
   *          index of initial character in str to convert
   * @param inlength
   *          number of characters to convert
   * @param work
  ...
  /**
   * Returns true when the converter has consumed some bytes that are not yet
   * converted to characters because further continuation bytes are needed.
   * Defaults to false, should be overridden by decoders that internally store
   * some bytes.
   */
