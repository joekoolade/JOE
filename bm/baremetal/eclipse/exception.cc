...
_Jv_Throw (jthrowable value)
{
  ...
  /*
    Get the return address and frame pointer
  */
  asm("mov 4(%%ebp), %[value]" : [value] "=r" (returnAddress));
  asm("mov (%%ebp), %[value]" : [value] "=r" (frame));
  asm("mov %%ebp, %0" : "=r" (stack));
  stack+=8;
  baremetal::runtime::Thrower::throwException(returnAddress, frame, stack, value);
...
// 		catch_type = (jclass)catch_word;
	      ...
	      if (_Jv_IsInstanceOf ((jobject)xh->value, catch_type))
		{
