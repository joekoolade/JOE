...
import baremetal.platform.ConsoleErrStream;
import baremetal.platform.ConsoleOutputStream;
...
public final class System {
    ...
    int size = 0;
    /*
     * todo: check src,dst as arrays
     */
    if(src==null || dest==null)
      throw new NullPointerException();
    
    int srcPtr = Memory.getAddress(src);
    int dstPtr = Memory.getAddress(dest);
    if(len<0 || srcStart<0 || srcStart+len>Array.get32(srcPtr,2) || destStart<0 || destStart+len>Array.get32(dstPtr,2))
    ...
    /*
     * todo: check the src and dest array types
     */
    java.lang.Class srcComp = Class.getComponentType(src.getClass());
    java.lang.Class destComp = Class.getComponentType(dest.getClass());
    if(baremetal.runtime.Class.isPrimitive(srcComp)) {
      size = baremetal.runtime.Class.getElementSize(srcComp);
    } else {
      size = 4;
    }
    
    Memory.bcopy(srcPtr+12+(srcStart*size), dstPtr+12+(destStart*size), len*size);
	}
	 ...
	 */
	public static boolean isWordsBigEndian() {
		return false;
	}
