    ...
    implements SelChImpl
{
        ...
        throws IOException
    {
        ...
        this.family = StandardProtocolFamily.INET;
        this.socket = new InetSocket();
        this.udp = Udp.udpFactory();
        ...
        throws IOException
    {
        ...
        synchronized (stateLock) {
            ...
            if (name == StandardSocketOptions.IP_TOS) {
                ...
                if (family == StandardProtocolFamily.INET) {
                    return (T) socket.getSocketOption(name);
                } else {
...
//            }
    ...
    private int receive0(ByteBuffer dst)
        throws IOException
    {
        ...
        if (dst instanceof DirectBuffer && rem > 0)
            return receiveIntoNativeBuffer(dst, rem, pos);
        ...
        try {
            int n = receiveIntoNativeBuffer(bb, newSize, 0);
            bb.flip();
            if (n > 0 && rem > 0)
                dst.put(bb);
            return n;
    ...
    private int receiveIntoNativeBuffer(ByteBuffer bb,
                                        int rem, int pos)
        throws IOException
    {
        int n = socket.receive(((DirectBuffer)bb).address() + pos, rem,
                         isConnected());
        if (n > 0)
            bb.position(pos + n);
        return n;
    ...
    private int send(ByteBuffer src, InetSocketAddress target)
        throws IOException
    {
        if (src instanceof DirectBuffer)
            return sendFromNativeBuffer(src, target);
        ...
        try {
            ...
            int n = sendFromNativeBuffer(bb, target);
            if (n > 0) {
                // now update src
                src.position(pos + n);
            }
    ...
    private int sendFromNativeBuffer(ByteBuffer bb,
        ...
        throws IOException
    {
        ...
        int written;
        try {
            written = socket.send(preferIPv6, ((DirectBuffer)bb).address() + pos,
        ...
        synchronized (readLock) {
            ...
            try {
                ...
                if (!isOpen())
                    return 0;
                do {
                    n = socket.read(dsts, offset, length);
                } while ((n == IOStatus.INTERRUPTED) && isOpen());
                return IOStatus.normalize(n);
            } finally {
                readerThread = 0;
        ...
        synchronized (writeLock) {
            ...
            try {
                ...
                if (!isOpen())
                    return 0;
                do {
                    n = socket.write(srcs, offset, length);
                } while ((n == IOStatus.INTERRUPTED) && isOpen());
                return IOStatus.normalize(n);
            } finally {
                writerThread = 0;
                ...
                synchronized (stateLock) {
                    ...
                    if (sm != null)
                        sm.checkConnect(isa.getAddress().getHostAddress(),
                                        isa.getPort());
                    int n = socket.connect(isa.getAddress(), isa.getPort());
                    if (n <= 0)
