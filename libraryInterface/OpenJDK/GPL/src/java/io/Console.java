...
public final class Console implements Flushable
{
   ...
   * {@link java.io.Reader#read(java.nio.CharBuffer) read(java.nio.CharBuffer)} on
   * the returned object will not read in characters beyond the line bound for
   * each invocation, even if the destination buffer has space for more
   * characters. A line bound is considered to be any one of a line feed
   ...
   *          Arguments referenced by the format specifiers in the format string.
   *          If there are more arguments than format specifiers, the extra
   *          arguments are ignored. The maximum number of arguments is limited by
   *          the maximum dimension of a Java array as defined by the
   *          <a href="http://java.sun.com/docs/books/vmspec/">Java Virtual
   ...
   *
   * @return A string containing the line read from the console, not including any
   *         line-termination characters, or <tt>null</tt> if an end of stream has
   *         been reached.
   */
   ...
   * @param fmt
   *          A format string as described in
   *          <a href="../util/Formatter.html#syntax">Format string syntax</a> for
   *          the prompt text.
   *
   * @param args
   *          Arguments referenced by the format specifiers in the format string.
   ...
   *           If an I/O error occurs.
   *
   * @return A character array containing the password or passphrase read from the
   *         console, not including any line-termination characters, or
  ...
  private static boolean echo(boolean on)
  {
    echoOff = !on;
    return on;
  }
        ...
        for (;;)
        {
          ...
          { // fill
            ...
            if (n > 0)
            {
              ...
              {
                /*
                 * we're in canonical mode so each "fill" should come back with an eol. if there
                 * no lf or nl at the end of returned bytes we reached an eof.
                 */
                eof = true;
              }
            } else
          ...
          while (nextChar < nChars)
          {
            ...
            } else if (c == '\r')
            {
              ...
              {
                /*
                 * no space left even the next is LF, so return whatever we have if the invoker
                 * is not our readLine()
                 */
                if (cbuf == rcb)
                {
              ...
              {
                /*
                 * we have a CR and we reached the end of the read in buffer, fill to make sure
                 * we don't miss a LF, if there is one, it's possible that it got cut off during
                 * last round reading simply because the read in buffer was full.
                 */
