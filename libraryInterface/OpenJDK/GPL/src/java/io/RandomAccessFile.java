...
public class RandomAccessFile implements DataOutput, DataInput, Closeable
{
   ...
   * a file with the specified name. A new {@link FileDescriptor} object is
   * created to represent the connection to the file.
   *
   * <p>
   * The <tt>mode</tt> argument specifies the access mode with which the file is
   * to be opened. The permitted values and their meanings are as specified for
   * the <a href="#mode"><tt>RandomAccessFile(File,String)</tt></a> constructor.
   *
   * <p>
   * If there is a security manager, its <code>checkRead</code> method is called
   * with the <code>name</code> argument as its argument to see if read access to
   * the file is allowed. If the mode allows writing, the security manager's
   * <code>checkWrite</code> method is also called with the <code>name</code>
   ...
   *              if the mode is <tt>"r"</tt> but the given string does not denote
   *              an existing regular file, or if the mode begins with
   *              <tt>"rw"</tt> but the given string does not denote an existing,
   *              writable regular file and a new regular file of that name cannot
   *              be created, or if some other error occurs while opening or
   *              creating the file
   * @exception SecurityException
   *              if a security manager exists and its <code>checkRead</code>
   *              method denies read access to the file or the mode is "rw" and
   *              the security manager's <code>checkWrite</code> method denies
  ...
  /**
   * Creates a random access file stream to read from, and optionally to write to,
   * the file specified by the {@link File} argument. A new {@link FileDescriptor}
   * object is created to represent this file connection.
   *
   * <a name="mode">
   * <p>
   * The <tt>mode</tt> argument specifies the access mode in which the file is to
   * be opened. The permitted values and their meanings are:
   *
   * <blockquote>
   ...
   * <td>Open for reading only. Invoking any of the <tt>write</tt> methods of the
   * resulting object will cause an {@link java.io.IOException} to be thrown.</td>
   * </tr>
   * <tr>
   * <td valign="top"><tt>"rw"</tt></td>
   * <td>Open for reading and writing. If the file does not already exist then an
   * attempt will be made to create it.</td>
   * </tr>
   * <tr>
   ...
   * to the underlying storage device.</td>
   * </tr>
   * <tr>
   ...
   * {@link java.nio.channels.FileChannel#force(boolean) force(boolean)} method of
   * the {@link java.nio.channels.FileChannel} class, passing arguments of
   * <tt>true</tt> and <tt>false</tt>, respectively, except that they always apply
   * to every I/O operation and are therefore often more efficient. If the file
   * resides on a local storage device then when an invocation of a method of this
   * class returns it is guaranteed that all changes made to the file by that
   * invocation will have been written to that device. This is useful for ensuring
   * that critical information is not lost in the event of a system crash. If the
   * file does not reside on a local device then no such guarantee is made.
   *
   * <p>
   * The <tt>"rwd"</tt> mode can be used to reduce the number of I/O operations
   * performed. Using <tt>"rwd"</tt> only requires updates to the file's content
   * to be written to storage; using <tt>"rws"</tt> requires updates to both the
   * file's content and its metadata to be written, which generally requires at
   * least one more low-level I/O operation.
   *
   * <p>
   * If there is a security manager, its <code>checkRead</code> method is called
   * with the pathname of the <code>file</code> argument as its argument to see if
   * read access to the file is allowed. If the mode allows writing, the security
   * manager's <code>checkWrite</code> method is also called with the path
   ...
   *              <tt>"rw"</tt>, <tt>"rws"</tt>, or <tt>"rwd"</tt>
   * @exception FileNotFoundException
   *              if the mode is <tt>"r"</tt> but the given file object does not
   ...
   *
   * <p>
   * The {@link java.nio.channels.FileChannel#position() </code>position<code>} of
   * the returned channel will always be equal to this object's file-pointer
   ...
   * read-write mode if the O_RDWR bit in <code>mode</code> is true, else the file
   * is opened as read-only. If the <code>name</code> refers to a directory, an
   * IOException is thrown.
   *
   * @param name
   *          the name of the file
   * @param mode
   *          the mode flags, a combination of the O_ constants defined above
   */
  private void open(String name, int mode) throws FileNotFoundException
  {
    jfd = new JavaFile(name, mode);
  }
   ...
   * the range 0 to 255 (<code>0x00-0x0ff</code>). This method blocks if no input
   * is yet available.
   * <p>
   ...
   *
   * @return the next byte of data, or <code>-1</code> if the end of the file has
   *         been reached.
   * @exception IOException
   ...
   *              of file, or if the random access file has been closed, or if
   ...
   * method is greater than the <code>newLength</code> argument then the file will
   ...
   * <p>
   * If the present length of the file as returned by the <code>length</code>
   * method is smaller than the <code>newLength</code> argument then the file will
   * be extended. In this case, the contents of the extended portion of the file
   * are not defined.
   *
   * @param newLength
   ...
   * or output operations and cannot be reopened.
   *
   * <p>
  ...
  public void close() throws IOException
  {
    ...
    {
      /*
       * Decrement FD use count associated with the channel. The FD use count is
    ...
    /*
     * Decrement FD use count associated with this stream. The count got incremented
     * by FileDescriptor during its construction.
     */
    fd.decrementAndGetUseCount();
    jfd.close();
  }
  ...
  /**
   * Reads a <code>boolean</code> from this file. This method reads a single byte
   * from the file, starting at the current file pointer. A value of
   ...
   * the file, starting from the current file pointer. If the byte read is
   ...
   * order, are <code>b1</code> and <code>b2</code>, where each of the two values
   * is between <code>0</code> and <code>255</code>, inclusive, then the result is
   * equal to: <blockquote>
   * 
   * <pre>
   * (short) ((b1 &lt;&lt; 8) | b2)
   ...
   * <code>int</code> value, starting at the current file pointer, as if by the
   ...
   * <code>long</code> value, starting at the current file pointer, as if by the
   ...
   * line terminator or the end of the file. Each byte is converted into a
   * character by taking the byte's value for the lower eight bits of the
   ...
   * end of the file. Line-terminating characters are discarded and are not
   ...
   * <p>
   * The first two bytes are read, starting from the current file pointer, as if
   ...
   * modified UTF-8 format and are converted into characters.
   * <p>
   ...
   *              if the bytes do not represent valid modified UTF-8 encoding of a
   *              Unicode string.
   ...
   * <code>false</code> is written out as the value <code>(byte)0</code>. The
  ...
  /**
   * Writes a <code>byte</code> to the file as a one-byte value. The write starts
  ...
  /**
   * Writes a <code>short</code> to the file as two bytes, high byte first. The
   * write starts at the current position of the file pointer.
   *
   * @param v
  ...
  /**
   * Writes an <code>int</code> to the file as four bytes, high byte first. The
   * write starts at the current position of the file pointer.
   *
   * @param v
  ...
  /**
   * Writes a <code>long</code> to the file as eight bytes, high byte first. The
   * write starts at the current position of the file pointer.
   *
   * @param v
   ...
   * writes that <code>int</code> value to the file as a four-byte quantity, high
   ...
   * writes that <code>long</code> value to the file as an eight-byte quantity,
   ...
   * written to the data output stream as if by the <code>writeChar</code> method.
   * The write starts at the current position of the file pointer.
   *
   * @param s
   ...
   * is output, in sequence, using the modified UTF-8 encoding for each character.
   *
   * @param str
