 ...
 * <li>
 * <p>
 * A method for {@link #compact compacting} a byte buffer.
 * </p>
 * </li>
 *
 * </ul>
 ...
 * primitive types, except {@code boolean}. Primitive values are translated to
 ...
 *  void  {@link #putFloat(int,float) putFloat(int index, float f)}</pre>
 * 
 * </blockquote>
 *
 * <p>
 * Corresponding methods are defined for the types {@code char,
 * short, int, long}, and {@code double}. The index parameters of the absolute
 * <i>get</i> and <i>put</i> methods are in terms of bytes rather than of the
 * type being read or written.
 *
 * <a id="views"></a>
 *
 * <p>
 ...
 * Corresponding view-creation methods are defined for the types
 * {@code char, short, int, long}, and {@code double}.
 *
 * <p>
...
public abstract class ByteBuffer extends Buffer implements Comparable<ByteBuffer>
{
     ...
     * its byte order will be {@link ByteOrder#BIG_ENDIAN BIG_ENDIAN}. Whether or
     * not it has a {@link #hasArray backing array} is unspecified.
     *
     * @param capacity The new buffer's capacity, in bytes
     *
     * @return The new byte buffer
     *
     * @throws IllegalArgumentException If the {@code capacity} is a negative
     *                                  integer
     */
     ...
     * buffer's capacity will be {@code array.length}, its position will be
     ...
     *               no larger than {@code array.length - offset}. The new buffer's
     *               limit will be set to {@code offset + length}.
     *
     * @return The new byte buffer
     *
     ...
     * The new buffer will be backed by the given byte array; that is, modifications
     * to the buffer will cause the array to be modified and vice versa. The new
     * buffer's capacity and limit will be {@code array.length}, its position will
     * be zero, its mark will be undefined, and its byte order will be
     * 
     * {@link ByteOrder#BIG_ENDIAN BIG_ENDIAN}.
     * 
     * 
     ...
     * the number of bytes remaining in this buffer, its mark will be undefined, and
     * its byte order will be
     * 
     * {@link ByteOrder#BIG_ENDIAN BIG_ENDIAN}.
     * 
     ...
     * 
     * and mark values will be identical to those of this buffer, and its byte order
     ...
     *
     * <p>
     * Otherwise, this method copies {@code length} bytes from this buffer into the
     ...
     * @return This buffer
     *
     * @throws BufferUnderflowException  If there are fewer than {@code length}
     ...
     * that is, if {@code length}&nbsp;{@code >}&nbsp;{@code remaining()}, then no
     * bytes are transferred and a {@link BufferOverflowException} is thrown.
     *
     * <p>
     ...
     *               be non-negative and no larger than {@code array.length}
     *
     * @param length The number of bytes to be read from the given array; must be
     *               non-negative and no larger than {@code array.length - offset}
     *
     * @return This buffer
     *
     * @throws BufferOverflowException   If there is insufficient space in this
     ...
     * this buffer. An invocation of this method of the form {@code dst.put(a)}
     ...
     *
     * <p>
     * If this method returns {@code true} then the {@link #array() array} and
     * {@link #arrayOffset() arrayOffset} methods may safely be invoked.
     * </p>
     *
     * @return {@code true} if, and only if, this buffer is backed by an array and
     *         is not read-only
     */
     ...
     * <i>p</i>&nbsp;=&nbsp;{@code position()} is copied to index zero, the byte at
     * index <i>p</i>&nbsp;+&nbsp;1 is copied to index one, and so forth until the
     * byte at index {@code limit()}&nbsp;-&nbsp;1 is copied to index
     ...
     * another via the buffer {@code buf}:
     *
     * <blockquote>
     * 
     * <pre>{@code
     ...
     * is, upon the elements from {@code position()} up to, and including, the
     * element at {@code limit()}&nbsp;-&nbsp;{@code 1}.
     *
     * <p>
     ...
     *
     * @return {@code true} if, and only if, this buffer is equal to the given
     *         object
     */
     ...
     * buffer and a given buffer. The index is relative to the {@link #position()
     * position} of each buffer and will be in the range of 0 (inclusive) up to the
     * smaller of the {@link #remaining() remaining} elements in each buffer
     * (exclusive).
     *
     * <p>
     * If the two buffers share a common prefix then the returned index is the
     * length of the common prefix and it follows that there is a mismatch between
     * the two buffers at that index within the respective buffers. If one buffer is
     * a proper prefix of the other then the returned index is the smaller of the
     * remaining elements in each buffer, and it follows that the index is only
     * valid for the buffer with the larger number of remaining elements. Otherwise,
     ...
     *         buffer, otherwise -1 if no mismatch.
     *
     * @since 11
     */
    ...
    /**
     * Returns the memory address, pointing to the byte at the given index, modulus
     * the given unit size.
     *
     * <p>
     * A return value greater than zero indicates the address of the byte at the
     * index is misaligned for the unit size, and the value's quantity indicates how
     * much the index should be rounded up or down to locate a byte at an aligned
     * address. Otherwise, a value of {@code 0} indicates that the address of the
     * byte at the index is aligned for the unit size.
     *
     * @apiNote This method may be utilized to determine if unit size bytes from an
     *          index can be accessed atomically, if supported by the native
     *          platform.
     *
     * @implNote This implementation throws {@code UnsupportedOperationException}
     *           for non-direct buffers when the given unit size is greater then
     *           {@code 8}.
     *
     * @param index    The index to query for alignment offset, must be
     *                 non-negative, no upper bounds check is performed
     *
     ...
     *
     * @throws IllegalArgumentException      If the index is negative or the unit
     *                                       size is not a power of {@code 2}
     *
     * @throws UnsupportedOperationException If the native platform does not
     *                                       guarantee stable alignment offset
     *                                       values for the given unit size when
     *                                       managing the memory regions of buffers
     *                                       of the same kind as this buffer (direct
     *                                       or non-direct). For example, if garbage
     *                                       collection would result in the moving
     *                                       of a memory region covered by a
     *                                       non-direct buffer from one location to
     *                                       another and both locations have
    ...
    /**
     * Creates a new byte buffer whose content is a shared and aligned subsequence
     * of this buffer's content.
     *
     * <p>
     * The content of the new buffer will start at this buffer's current position
     * rounded up to the index of the nearest aligned byte for the given unit size,
     * and end at this buffer's limit rounded down to the index of the nearest
     * aligned byte for the given unit size. If rounding results in out-of-bound
     * values then the new buffer's capacity and limit will be zero. If rounding is
     ...
     * <p>
     * The new buffer's position will be zero, its capacity and its limit will be
     * the number of bytes remaining in this buffer or fewer subject to alignment,
     ...
     *          bytes from index, that is a multiple of the unit size, may be
     *          accessed atomically, if supported by the native platform.
     *
     * @implNote This implementation throws {@code UnsupportedOperationException}
     *           for non-direct buffers when the given unit size is greater then
     *           {@code 8}.
     ...
     * @return The new byte buffer
     *
     * @throws IllegalArgumentException      If the unit size not a power of
     *                                       {@code 2}
     *
     * @throws UnsupportedOperationException If the native platform does not
     *                                       guarantee stable aligned slices for the
     *                                       given unit size when managing the
     *                                       memory regions of buffers of the same
     *                                       kind as this buffer (direct or
     *                                       non-direct). For example, if garbage
     *                                       collection would result in the moving
     *                                       of a memory region covered by a
     *                                       non-direct buffer from one location to
     *                                       another and both locations have
     ...
     * <p>
     * The new buffer's position will be zero, its capacity and its limit will be
     * the number of bytes remaining in this buffer divided by two, its mark will be
     * undefined, and its byte order will be that of the byte buffer at the moment
     * the view is created. The new buffer will be direct if, and only if, this
     ...
     * <p>
     * The new buffer's position will be zero, its capacity and its limit will be
     * the number of bytes remaining in this buffer divided by four, its mark will
     * be undefined, and its byte order will be that of the byte buffer at the
     * moment the view is created. The new buffer will be direct if, and only if,
     ...
     * <p>
     * The new buffer's position will be zero, its capacity and its limit will be
     * the number of bytes remaining in this buffer divided by eight, its mark will
     * be undefined, and its byte order will be that of the byte buffer at the
     * moment the view is created. The new buffer will be direct if, and only if,
     * this buffer is direct, and it will be read-only if, and only if, this buffer
     * is read-only.
