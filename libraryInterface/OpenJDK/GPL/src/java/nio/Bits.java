...
class Bits {                            // package-private
    ...
    private static byte _get(long a) {
        return Address.fromLong(a).loadByte();
    }

    private static void _put(long a, byte b) {
        Address.fromLong(a).store(b);
    }
    ...
    static int pageSize() {
        if (pageSize == -1)
            pageSize = BootRecord.the_boot_record.bytesInPage.toInt();
        return pageSize;
    }
    ...
    private static volatile long maxMemory = HeapLayoutConstants.MAXIMUM_MAPPABLE.toLong();
