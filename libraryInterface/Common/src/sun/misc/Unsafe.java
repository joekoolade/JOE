...
import static org.jikesrvm.mm.mminterface.Barriers.NEEDS_BOOLEAN_GETFIELD_BARRIER;
import static org.jikesrvm.mm.mminterface.Barriers.NEEDS_BOOLEAN_PUTFIELD_BARRIER;
import static org.jikesrvm.mm.mminterface.Barriers.NEEDS_BYTE_GETFIELD_BARRIER;
...
import static org.jikesrvm.mm.mminterface.Barriers.NEEDS_DOUBLE_GETFIELD_BARRIER;
import static org.jikesrvm.mm.mminterface.Barriers.NEEDS_DOUBLE_PUTFIELD_BARRIER;
import static org.jikesrvm.mm.mminterface.Barriers.NEEDS_FLOAT_GETFIELD_BARRIER;
import static org.jikesrvm.mm.mminterface.Barriers.NEEDS_FLOAT_PUTFIELD_BARRIER;
import static org.jikesrvm.mm.mminterface.Barriers.NEEDS_INT_GETFIELD_BARRIER;
import static org.jikesrvm.mm.mminterface.Barriers.NEEDS_INT_PUTFIELD_BARRIER;
import static org.jikesrvm.mm.mminterface.Barriers.NEEDS_LONG_GETFIELD_BARRIER;
...
import static org.jikesrvm.mm.mminterface.Barriers.NEEDS_SHORT_GETFIELD_BARRIER;
import static org.jikesrvm.mm.mminterface.Barriers.NEEDS_SHORT_PUTFIELD_BARRIER;
import static org.jikesrvm.mm.mminterface.Barriers.booleanFieldRead;
import static org.jikesrvm.mm.mminterface.Barriers.booleanFieldWrite;
import static org.jikesrvm.mm.mminterface.Barriers.byteFieldRead;
import static org.jikesrvm.mm.mminterface.Barriers.byteFieldWrite;
import static org.jikesrvm.mm.mminterface.Barriers.charFieldRead;
import static org.jikesrvm.mm.mminterface.Barriers.charFieldWrite;
import static org.jikesrvm.mm.mminterface.Barriers.doubleFieldRead;
import static org.jikesrvm.mm.mminterface.Barriers.doubleFieldWrite;
import static org.jikesrvm.mm.mminterface.Barriers.floatFieldRead;
import static org.jikesrvm.mm.mminterface.Barriers.floatFieldWrite;
import static org.jikesrvm.mm.mminterface.Barriers.intFieldRead;
import static org.jikesrvm.mm.mminterface.Barriers.intFieldWrite;
import static org.jikesrvm.mm.mminterface.Barriers.longFieldRead;
import static org.jikesrvm.mm.mminterface.Barriers.longFieldWrite;
import static org.jikesrvm.mm.mminterface.Barriers.objectFieldRead;
import static org.jikesrvm.mm.mminterface.Barriers.objectFieldWrite;
import static org.jikesrvm.mm.mminterface.Barriers.shortFieldRead;
import static org.jikesrvm.mm.mminterface.Barriers.shortFieldWrite;
...
/**
 * Our implementation of sun.misc.Unsafe maps the operations to the Jikes RVM
 * compiler intrinsics and its runtime service methods.
 * <p>
 * The OpenJDK class libraries (and normal programs using sun.misc.Unsafe) may
 * expect that the methods in this class are compiler intrinsics (because that's
 * the case for HotSpot). Therefore, force inlining of all methods where
 ...
 * appropriate type. Methods without such a parameter are assumed to access
 * native memory and thus don't have barriers. Note that HotSpot's
 * {@code sun.misc.Unsafe} doesn't make any distinction between barriers for
 * field accesses and those for array accesses. The put* and get* methods in
 * this class use barriers for field accesses.
 */
public final class Unsafe
{

  /**
   * use name that Doug Lea's Fork-Join framework appears to expect from class
   * libs
   */
  ...
  public long allocateMemory(long bytes)
  {
    ...
    Address result = Magic.objectAsAddress(b);
    if (result.isZero())
    {
  ...
  @Inline
  public Class<?> defineClass(String name, byte[] bytes, int off, int len, final ClassLoader parentClassLoader,
  ProtectionDomain protectionDomain)
  {
    if (parentClassLoader != null)
    {
      return RVMClassLoader.defineClassInternal(name, bytes, off, len, parentClassLoader, protectionDomain)
      .getClassForType();
    } else
    {
      ...
      + "from VMStackWalker.getCallingClassLoader() to a place that's not in the GNU " + "Classpath namespace");
      return RVMClassLoader.defineClassInternal(name, bytes, off, len, callingClassloader, protectionDomain)
      .getClassForType();
    }
  }
  ...
  // FIXME OPENJDK/ICEDTEA add atomic copying if size is aligned properly (i.e. 8
  // / 4 / 2 bytes).
  @Inline
  public void copyMemory(long srcAddress, long destAddress, long bytes)
  {
    Memory.memcopy(Address.fromLong(destAddress), Address.fromLong(srcAddress),
    Offset.fromLong(bytes).toWord().toExtent());
  }
  ...
  @Inline
  @Deprecated // deprecated in OpenJDK because it returns an int as "offset" and OpenJDK wants
              // to use longs
  ...
  @Inline
  @Deprecated // deprecated in OpenJDK because it uses an int as "offset" and OpenJDK wants to
              // use longs
