...
public class DHCPPacket implements Cloneable, Serializable {
    ...
    final private static Offset OP_OFFSET = Offset.zero();
    final private static Offset HTYPE_OFFSET = Offset.fromIntSignExtend(1);
    final private static Offset HLEN_OFFSET = Offset.fromIntSignExtend(2);
    final private static Offset HOPS_OFFSET = Offset.fromIntSignExtend(3);
    final private static Offset XID_OFFSET = Offset.fromIntSignExtend(4);
    final private static Offset SECS_OFFSET = Offset.fromIntSignExtend(8);
    final private static Offset FLAGS_OFFSET = Offset.fromIntSignExtend(10);
    final private static Offset CIADDR_OFFSET = Offset.fromIntSignExtend(12);
    final private static Offset YIADDR_OFFSET = Offset.fromIntSignExtend(16);
    final private static Offset SIADDR_OFFSET = Offset.fromIntSignExtend(20);
    final private static Offset GIADDR_OFFSET = Offset.fromIntSignExtend(24);
    final private static Offset CHADDR_OFFSET = Offset.fromIntSignExtend(28);
    final private static Offset SNAME_OFFSET = Offset.fromIntSignExtend(44);
    final private static Offset FILE_OFFSET = Offset.fromIntSignExtend(108);
    final private static Offset OPTIONS_OFFSET = Offset.fromIntSignExtend(236);
...
//		h += this.truncated ? 1 : 0;
    ...
    protected DHCPPacket marshall(byte[] buffer, int offset, int length,
        ...
        // get the address
        Address bufferAddr = Magic.objectAsAddress(buffer);
        // parse static part of packet
        this.op    = bufferAddr.loadByte();
        this.htype = bufferAddr.loadByte(HTYPE_OFFSET);
        this.hlen  = bufferAddr.loadByte(HLEN_OFFSET);
        this.hops  = bufferAddr.loadByte(HOPS_OFFSET);
        this.xid   = ByteOrder.networkToHost(bufferAddr.loadInt(XID_OFFSET));
        this.secs  = ByteOrder.networkToHost(bufferAddr.loadShort(SECS_OFFSET));
        this.flags = ByteOrder.networkToHost(bufferAddr.loadShort(FLAGS_OFFSET));
        copy(buffer, CIADDR_OFFSET, this.ciaddr);
        copy(buffer, YIADDR_OFFSET, this.yiaddr);
        copy(buffer, SIADDR_OFFSET, this.siaddr);
        copy(buffer, GIADDR_OFFSET, this.giaddr);
        ...
        if (ByteOrder.networkToHost(bufferAddr.loadInt(OPTIONS_OFFSET)) != _MAGIC_COOKIE) {
            this.isDhcp = false;
        }
        ...
        if (this.isDhcp) {    // is it a full DHCP packet or a simple BOOTP?
            ...
            while (true) {
                ...
                r = bufferAddr.loadByte(index);
                index = index.plus(1);
                if (r < 0) { break; } // EOF

                int    len      = r; // Math.min(r, inBStream.available());
                byte[] unit_opt = new byte[len];
                // read in values
                for(int i=0; i < len; i++)
                {
                    unit_opt[i] = bufferAddr.loadByte(index);
                    index = index.plus(1);
                }
        ...
        // read in the rest
        for(int i=0; i < padding.length && index.toInt() < DHCPConstants.MAX_LEN; i++)
        {
            padding[i] = bufferAddr.loadByte(index);
            index = index.plus(1);
        }
    ...
    final private void copy(byte[] src, Offset offset, byte[] dst)
    {
        System.arraycopy(src, offset.toInt(), dst, 0, dst.length);
    }
    /**
