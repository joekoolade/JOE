...
static char* GetMethodName(JavaMethod* methodDef,
                           Class* customizeFor) {
  ...
  char* buf = new char[3 + JNI_NAME_PRE_LEN + 1 + ((mnlen + clen + mtlen) << 3)];
...
Function* LLVMMethodInfo::getMethod(Class* customizeFor) {
  ...
  if (result == NULL) {
    if (Compiler->emitFunctionName()) {
      char* buf = GetMethodName(methodDef, customizing ? customizeFor : NULL);
...
void LLVMMethodInfo::setCustomizedVersion(Class* cl, llvm::Function* F) {
  ...
  if (Compiler->emitFunctionName()) {
    char* buf = GetMethodName(methodDef, cl);
    F->setName(buf);
  }
  methodFunction = NULL;
