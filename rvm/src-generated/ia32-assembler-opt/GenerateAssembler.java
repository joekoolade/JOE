 ...
 * compiler tables. The stylization of the Assembler side is more thoroughgoing,
 * and the reader is referred to the Assembler header comments for a definition.
 * </P>
 *
 * <P>
 ...
 * InstructionFormatTable and OperatorFormatTable. It then, for each operator,
 * generates a handler method to call the appropriate Assembler emit method
 * given an Instruction. The Assembler will have a family of emit methods named
 * for each opcode, each such emit method takes a specific set of operand
 * addressing modes and sizes. The handler methods that the GenerateAssembler
 * emits examine the operands to an Instruction, and determine which Assembler
 * method to call for the operand addressing modes and sizes that it finds.
 * GenerateAssembler also generates a top-level dispatch method that examines
 * the operator and calls the appropriate handler.
 * </P>
 *
 * <P>
 ...
 * Assembler via reflection to generate the Assembler, but that is not possible
 * until the VM sources (including, of course, the Assembler) have been
 * compiled. The current hack to get around this is to compile the Assembler in
 * advance, and read the resulting class file. This utilizies some supporting
 * files to make the Assembler compile in isolation. This is the purpose of the
 * .fake files in the optimizing compiler's assembler directory.
 * </P>
 *
 * <P>
...
public class GenerateAssembler
{
     ...
     * descriptions of each optimizing compiler instruction format that sis visible
     * to the assembler (i.e. the MIR_* instruction formats.
     *
     * @see InstructionFormatTables
     */
     ...
     * processed. This table is null unless some of the operands in the Instruction
     * are to ignored when generating code for the opcode. Ignoring arguments is an
     * ad-hock special case that is controlled by the global opcodeArgTable.
     */
     ...
     * well, but there are exceptions in that multiple IA32_* opcodes can map to the
     * same IA32 instruction
     */
    static String currentOpcode;

    /**
     * The instruction format for the IA32_* opt compiler opcode(s) being processed.
     */
    static String currentFormat;

    /**
     * Global table mapping opt compiler IA32_* opcodes to arrays listing the set of
     * Instruction operands that are to be used as arguments to the IA32
     * architecture instruction. This is used when an instruction has extra operands
     * that are not used in assembly. The array is indexed by the desired argument
     * for the instruction/Assembler method, the value in the array states which
     * operand of the intruction contains the operand for the instruction. For
     * example, an array of "new int{2}" means the instruction has 1 operand and it
     * is read from the 2 operand of the instruction.
     */
    ...
    static
    {
        ...
        opcodeArgTables.put("CALL", new int[]
        { 2 });
        opcodeArgTables.put("INT", new int[]
        { 1 });
        opcodeArgTables.put("CDQ", new int[]
        { 0 });
        opcodeArgTables.put("CDO", new int[]
        { 0 });
        opcodeArgTables.put("CDQE", new int[]
        { 0 });
        opcodeArgTables.put("DIV", new int[]
        { 1, 2 });
        opcodeArgTables.put("IDIV", new int[]
        { 1, 2 });
        opcodeArgTables.put("MUL", new int[]
        { 1, 2 });
        opcodeArgTables.put("IMUL1", new int[]
        { 1, 2 });
        opcodeArgTables.put("DIV", new int[]
        { 1, 2 });
        opcodeArgTables.put("IDIV", new int[]
        { 1, 2 });
        opcodeArgTables.put("SET", new int[]
        { 1, 0 });
        opcodeArgTables.put("CMPXCHG", new int[]
        { 1, 2 });
        opcodeArgTables.put("CMPXCHG8B", new int[]
        { 2 });
        opcodeArgTables.put("FCMOV", new int[]
        { 2, 0, 1 });
        opcodeArgTables.put("CMOV", new int[]
        { 2, 0, 1 });
    }
    ...
    enum ArgumentType
    {
        ...
        Immediate("Imm"),
        /**
         * Constant representing register arguments to Assembler calls. This covers the
         * cases when a general purpose register is encoded into the mod/rm byte; the
         * Assembler handles the detais of generating either the reg bits of the mod/rm
         * byte or encoding a register as mod 11.
         */
        GPRegister("Reg", "GPR_Reg"), FPRegister("Reg", "FPR_Reg"), MMRegister("Reg", "MM_Reg"),
        XMMRegister("Reg", "XMM_Reg"),
        /**
         * Constant representing condition arguments to Assembler calls. Such operands
         * are not arguments to the ultimate IA32 machine code instruction, but they are
         * used to calculate the opcode that is generated.
         */
        Condition("Cond"),
        /**
         * Constant representing arguments to Assembler calls that use the
         * scaled-index-base (SIB) addressing mode in the special way that uses neither
         * a base not an index to generate an absolute address
         */
        Absolute("Abs"),
        /**
         * Constant representing IA32 memory operands that use register- displacement
         * addressing mode (usually mod bits 01 and 10) arguments to Assembler calls.
         * The Assembler takes care of choosing the right mode for the size of the
         * displacement, so this one mode covers two of the four addressing modes the
         * IA32 has. The Assembler also handles the special cases in which this mode
         * requires weird SIB bytes.
         */
        RegisterDisplacement(2, "RegDisp"),
        /**
         * Constant representing arguments to Assembler calls that use the
         * scaled-index-base (SIB) addressing mode in the special way that does not use
         * a base register. The Assembler simply assumes it has an [index < < scale +
         * disp] addressing mode, and the Assembler takes care of generating the special
         * mod/rm that causes the base register to be ignored.
         */
        RegisterOffset(3, "RegOff"),
        /**
         * Constant representing scaled-index-base (SIB) mode arguments to Assembler
         * calls.
         */
        RegisterIndexed(4, "RegIdx"),
        /**
         * Constant representing register-indirect arguments to Assembler calls. This
         * mode handles what is (usually) mod 00 in the mod/rm byte.
         */
        RegisterIndirect("RegInd"),
        /**
         * Constant representing labels used as branch targets. While code is being
         * generated, the machine code offset for a forward branch cannot, in general,
         * be computed as the target code has not been generated yet. The Assembler uses
         * synthetic code offsets, based upon the order of Instructions in the code
         * being compiled, to communicate forward branch targets to the Assembler. These
         * synthetic offsets are passed to the Assembler where it expected Label
         * arguments.
         */
        Label("Label"),
        /**
         * Constant representing arguments to Assembler calls in which it may be either
         * a backward branch target (resolved to an immediate being the exact branch
         * displacement) or a forward branch (which will be a synthetic Label).
         */
         ...
         *
         * @param name string used as part of assembler emit method name or for is and
         *             get methods in AssemblerBase
         */
        ...
        /**
         * @return does this argument type encode a size information for the instruction
         */
        boolean isSize()
        {
            return size != -1;
    ...
    /**
     * For a given string representing a valid operand encoding for the Assembler,
     * return the corresponding Assembler constant. This function only looks for
     * encodings of operand types, and will not accept strings that correspond to
     * size encodings.
     *
    ...
    private static ArgumentType getEncoding(String str, Class<?> type)
    {
        ...
        throw new Error("Unable to encode the argument " + str + " of type " + type + " as a valid argument type");
    }

    /**
     * For a given operand number, return a string which is a valid Java expression
     * for reading that operand out of the current instruction. This function uses
     * the currentOpcodeSymbolicNames table to determine the appropriate accessor
     * (e.g. getValue if the current name is Value), and it uses the
     * currentOpcodeArgTable (in cases where it has an entry for the kind of
     * instruction being processed) to determine which operand in Instruction
     * corresponds to operand sought.
     *
    ...
    /**
     * Given an operand number and an encoding, generate a test to determine whether
     * the given operand matches the encoding. That is, generate code to the
     * Assembler that examines a given operand of the current Instruction, and
     * determines whether it is of the type encoded by the given encoding. This is
     * used to generate the if statements of the dispatch functions for each opt
     * compiler opcode.
     *
    ...
    /**
     * Generate code to verify that a given operand matches a given encoding. Since
     * the IA32 architecture is not exactly orthogonal (please note the charitable
     * understatement), there are cases when the opt assembler can determine the
     * Assembler emitter to call without looking at all (or, in some cases, any) of
     * the arguments of the Instruction. An example is the ENTER instruction that
     * only takes one immediate parameter, so the opt assembler could simply call
     * that Assembler emiiter without checking that argument is really an immediate.
     * In such cases, the opt assembler generates guarded tests that verify that
     * Instruction operand actually matches the required encoding. This function
     * emits such tests to the assembler being generated.
     *
     * @param argNumber   The argument to examine
     * @param argEncoding The encoding for which to check
     * @param level       current level for generating pretty, tabified output
     */
    private static void emitVerify(int argNumber, ArgumentType argEncoding, int level)
    {
        emitTab(level);
     ...
     * appropriate Instruction argument and fetches of the appropriate pieces of
    ...
    /**
     * An EmitterDescriptor represents a single emit method from the Assembler: it
     * explicitly represents the types of operands the method expects, their number,
     * and the size of the data it uses. When GenerateAssembler encounters an emit*
     * method from the Assembler, it creates an EmitterDescriptor for it. Based upon
     * the stlyized form the method name is required to have, the EmitterDexcriptor
     * represents information about its arguments. This information is stored in
     * terms of the GenerateAssembler constants that represent operand type and
     * size.
     * <P>
     * The EmitterDescriptor class encapsulates the logic for parsing the stylized
     * emit* method names that the Assembler has, and turning them into the explicit
     * representation that GenerateAssembler uses. If parsing a name fails, a
     * {@link GenerateAssembler.BadEmitMethod} runtime exception is thrown and
     * assembler generation is aborted.
     * <P>
     * <HR>
    ...
    static class EmitterDescriptor
    {
         ...
         * operands of the given emit* method. This constructor encapsulate the logic to
         * parse the given method name into the appropriate explicit representation.
         */
         ...
         * argument'th operand. If enc is an operand type encoding, this method checks
         * wether the given argument is of the appropriate type. If enc is an operand
         * size encoding, the argument parameter is ignored, and this method checks
         * whether the emit* method represented operates upon data of the desired size.
         * <P>
         * <EM>See the descriptions of the GenerateAssembler constants:</EM>
         * <DL>
         * <DT><EM>Operand types</EM> <DI>
         ...
         *
         * @return True if this method expects an argument type encoded by enc as its
         *         argument'th operand, and false otherwise.
         */
    ...
    /**
     * An EmitterSet represents a set of emit methods from the Assembler for the
     * same IA32 assembler opcode. These sets are used when generating the
     * do<opcode> method for a given IA32 opcde: first an EmitterSet of all the
     * Assembler emit methods for that opcode is built, and then the do method is
     * recursively generated by emitting operand type and size tests that partition
     * the set of emitters into two smaller sets. This continues until the set is a
     * singleton
     */
    static class EmitterSet
    {
        ...
        /**
         * Count how many of the emit represented by this set match a given operand type
         * and size encoding. This method is used (via getEncodingSplit) while
         * recursively partitioning a given EmitterSet to determine how evenly (or even
         * whether) a given operand type and size splits this set.
         *
         ...
         * match a given operand type and size for a given operand, and the number of
         * those that do not. This method is used while recursively partitioning a given
         * EmitterSet to determine how evenly (or even whether) a given operand type and
         * size splits this set.
         *
         * @param n    the operand being examined
         * @param code the operand type or size code being considered
         * @return the different between matching and non-matching emit method in this
         *         set.
         */
        ...
        /**
         * This class is used just to communicate the two results of searching for the
         * best split for a given set: the chosen operand type or size, and the chosen
         * operand nummber. This class is basically to avoid writing the slew of
         * required type casts that a generic pair would need given Java's primitive
         * type system.
         *
         * @see #makeSplit
         * @see #split
        ...
        static class SplitRecord
        {
            ...
            /**
             * Make s split record to communicate the results of searching for the best
             * operand to split.
             *
             * argument The operand number to be split. test The operand type or size test
             * on which to split.
             */
        ...
        private EmitterSet[] makeSplit(SplitRecord split)
        {
            ...
            return new EmitterSet[]
            { yes, no };
        }

        /**
         * Find the best operand type or size and operand number to partition this
         * EmitterSet. This method searches across all possible ways of splitting this
         * set--all possible operand types and sizes, and all possible operands--to
         * determine which one splits the set most evenly.
         *
        ...
        /**
         * Emit the Java code to call a particular emit method for a particular opcode.
         * This method takes representations of the opcode and operands of a given emit
         * method, and generates the appropriate Java source code to call it. It
         * synthesizes the encoded emit method name, and uses emitArgs to pass all the
         * required arguments.
         *
         * @see #emitArgs
         *
         ...
         */
        private void emitEmitCall(String opcode, ArgumentType[] args, int count, int level, ArgumentType size)
        {
            if (DEBUG)
            {
        ...
        /**
         * Write the Java code required for error checking and calling the emit method
         * represented by a singleton EmitterSet. A singleton EmiiterSet will typically
         * be the result of a series of splits of bigger sets, where the splits
         * represent emitted queries of operand types and sizes. (See emitSet) However,
         * there may be cases when some operand has only one possible options, so the
         * splitting will not have generated any tests for it. In this case, we will
         * emit assertions that guarantee the operand is of the expected type. Note that
         * the answers to queries alrrready performed by splitting are known to be fine,
         * so no additional error checking is needed for cases they cover.
         *
         * @see #emitSet
         *
        ...
        /**
         * Emit Java code for deciding which emit method in the given set applies to an
         * Instruction, and then calling the apprpriate method. The method essentially
         * works by recursively parititioning the given set into two smaller pieces
         * until it finds a set with only one element. On each partition, this method
         * generates code for the appropriate operand type or size query, and then calls
         * itself recursively on the two sets resulting from the partition.
         *
         ...
         * splitting; this is passed to emitSingleton to help it generate appropriate
        ...
        private void emitSet(String opcode, boolean[][] testsPerformed, int level)
        {
            ...
            else
            {
                ...
                {
                    throw new Error(
                            "repeated split of " + opcode + "[" + rec.argument + "] for " + rec.test + "\n" + this);
                }
     ...
     * compiler (NOP is a good example) but may be present in the Assembler if other
     * compilers use them. We keep an explicit list of such opcodes to ignore.
     */
...
//        excludedOpcodes.add("LIDT");
    ...
    /**
     * Compute the set of all IA32 opcodes that have emit methods in the Assembler.
     * This method uses the stylized form of all emit method names in the Assembler
     * to extract the opcode of each one. It returns a set of all such distinct
    ...
    /**
     * returns a list of all IA32_ opt compiler operators that do not correspond to
     * real IA32 opcodes handled by the assembler. These are all supposed to have
     * been removed by the time the assembler is called, so the assembler actually
     * seeing such an opcode is an internal compiler error. This set is used during
     ...
     * useless. However, there are some special cases, notably for operand size. In
     * this case, an opt operator of the form ADD__B would mean use the ADD IA32
    ...
    public static void main(String[] args)
    {
        ...
        // Kludge for RDTSC
        emittedOpcodes.add("RDTSC");
        emitTab(3);
        emit("case IA32_RDTSC_opcode:\n");
        emitTab(4);
        emit("emitRDTSC();\n");
        emitTab(4);
        emit("break;\n");
        emittedOpcodes.add("PREFETCHNTA");
        emitTab(3);
        emit("case IA32_PREFETCHNTA_opcode:\n");
        emitTab(4);
        emit("break;\n");
        emittedOpcodes.add("PAUSE");
        emitTab(3);
        emit("case IA32_PAUSE_opcode:\n");
        emitTab(4);
        emit("break;\n");
        emitTab(3);
