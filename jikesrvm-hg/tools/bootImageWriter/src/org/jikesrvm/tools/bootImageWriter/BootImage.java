...
implements BootImageWriterConstants, BootImageInterface, SizeConstants
{
    ...
    /**
     * Number of object addresses set to null because they referenced objects that
     * are not part of bootimage
     */
    ...
    public void createSymbolTable()
    {
        ...
        Statics.bootImageReportGeneration(BootImageWriter.staticsJunk);
       for (int jtocSlot = Statics.middleOfTable, n = Statics.getHighestInUseSlot(); jtocSlot <= n; jtocSlot += Statics
        .getReferenceSlotSize())
        {
            ...
            Object obj = BootImageMap.getObject(BootImageWriter.getIVal(jtocOff));
            Address address = (BootImageWriter.getReferenceAddr(jtocOff, false));
            if (address.toInt() == 0)
                continue;
            ...
            else 
            {
                if (obj instanceof Class) {
                  ...
                  CompiledMethod m = BootImageWriter.findMethodOfCode(obj);
                  if (m != null) {
 ...
 //           say(symbol0 + ":" + symbol + ":" + Integer.toHexString(address.toInt()));
    ...
    public void write() throws IOException
    {
        ...
        /*
         * Now we generate a compressed reference map. Typically we get 4 bits/address,
         * but we'll create the in-memory array assuming worst case 1:1 compression.
         * Only the used portion of the array actually gets written into the image.
         */
     ...
     *
     * @param klass
     *            RVMClass object of scalar being allocated
     * @param needsIdentityHash
     *            needs an identity hash value
     * @param identityHashValue
     ...
     *
     * @param array
     *            RVMArray object of array being allocated.
     * @param numElements
     *            number of elements
     * @param needsIdentityHash
     *            needs an identity hash value
     ...
     *            the value for the identity hash
     * @param alignment
     *            special alignment value
     * @param alignCode
     *            Alignment-encoded value (AlignmentEncoding.ALIGN_CODE_NONE for
     *            none)
     * @return address of object within bootimage
     */
     ...
     */
    public Address allocateArray(RVMArray array, int numElements, boolean needsIdentityHash, int identityHashValue,
    int align, int alignCode)
    {
        numObjects++;
        BootImageWriter.logAllocation(array, array.getInstanceSize(numElements));
        return ObjectModel.allocateArray(this, array, numElements, needsIdentityHash, identityHashValue, align,
        alignCode);
    }
    ...
    /**
     * Reset the allocator as if no allocation had occured. This is useful to allow
     * a "trial run", as is done to establish the offset of the JTOC for the entry
     * in the boot image record---so its actual address can be computed early in the
     * build process.
     */
    ...
    /**
     * Set a byte in the reference bytemap to indicate that there is an address in
     * the boot image at this offset. This can be used for relocatability and for
     * fast boot image scanning at GC time.
     *
     * @param address
     *            The offset into the boot image which contains an address.
     */
    private void markReferenceMap(Address address)
    {
        ...
        if (referenceMap[referenceIndex] == 0)
        {
            ...
            if (referenceIndex > referenceMapLimit)
                referenceMapLimit = referenceIndex;
        }
    }
    ...
    public void setAddressWord(Address address, Word value, boolean objField, boolean root)
    {
        if (VM.VerifyAssertions)
            VM._assert(!root || objField);
        if (objField)
            value = MemoryManager.bootTimeWriteBarrier(value);
        if (root)
     ...
     *            address of target
     * @param objField
     *            true if this word is an object field (as opposed to a static, or
     *            TIB, or some other metadata)
     * @param root
     *            Does this slot contain a possible reference into the heap?
     *            (objField must also be true)
     * @param genuineNull
     *            true if the value is a genuine null and shouldn't be counted as a
     *            blanked field
     */
