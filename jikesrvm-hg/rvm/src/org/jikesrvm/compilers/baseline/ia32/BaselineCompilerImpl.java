...
import org.jikesrvm.compilers.common.assembler.ForwardReference.ConditionalBranch;
...
//      asm.emitINT_Imm(RuntimeEntrypoints.TRAP_CHECKCAST );
      asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.raiseClassCastException.getOffset()));
      fr.resolve(asm);
    }
...
//    asm.emitINT_Imm(RuntimeEntrypoints.TRAP_CHECKCAST);
  ...
  private void genPrologue() {
    ...
    if (method.hasInterruptHandlerAnnotation()) {
      ...
      if(VM.buildFor32Addr()) asm.emitMOV_RegDisp_Reg(TR, ArchEntrypoints.framePointerField.getOffset(), SP); // establish new frame
      else asm.emitMOV_RegDisp_Reg_Quad(TR, ArchEntrypoints.framePointerField.getOffset(), SP); // establish new frame
      /*
